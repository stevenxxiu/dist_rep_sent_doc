...
def main():
    ...
    arg_parser.add_argument('sg', type=bool)
    ...
    for i in itertools.count(0):
        ...
        with suppress(FileExistsError), open(name, 'x'), open(name, 'a', encoding='utf-8') as out:
            ...
            subprocess.call([sys.executable, 'dist_rep_sent_doc/main.py', args.dataset, 'val', 'dbow', json.dumps({
                'sg': p['sg'] if args.sg else 0, 'embedding_size': p['embedding_size'], 'min_freq': p['min_freq'],
                'sample': p['sample'], 'lr': p['train_lr'], 'batch_size': 2048, 'epoch_size': p['train_epoch_size'],
                'save_path': train_path
            })], stdout=out)
            subprocess.call([sys.executable, 'dist_rep_sent_doc/main.py', args.dataset, 'val', 'dbow', json.dumps({
                'sg': p['sg'] if args.sg else 0, 'embedding_size': p['embedding_size'], 'min_freq': p['min_freq'],
                'sample': p['sample'], 'lr': p['val_lr'], 'batch_size': 2048, 'epoch_size': p['val_epoch_size'],
