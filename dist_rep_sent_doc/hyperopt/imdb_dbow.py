...
def main():
    ...
    for i in itertools.count(0):
        name = f'results/results_{i:03d}.txt'
        with suppress(FileExistsError), open(name, 'x'), open(name, 'a', encoding='utf-8') as out:
            ...
            subprocess.call([sys.executable, 'dist_rep_sent_doc/main.py', 'imdb', 'val', 'dbow', json.dumps({
            ...
            subprocess.call([sys.executable, 'dist_rep_sent_doc/main.py', 'imdb', 'val', 'dbow', json.dumps({
                ...
                'save_path': '__cache__/tf/dbow_imdb_val', 'train_path': '__cache__/tf/dbow_imdb_train'
            })], stdout=out)
            subprocess.call([sys.executable, 'dist_rep_sent_doc/main.py', 'imdb', 'val', 'nn', json.dumps({
                'train_paths': ['__cache__/tf/dbow_imdb_train'],
                'test_paths': ['__cache__/tf/dbow_imdb_val'],
